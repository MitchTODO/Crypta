...
struct PollView: View {
    ...
    var body: some View {
        ...
        ZStack{
            NavigationView {
                List(pollVM.choices,id: \.id, selection: $selection) { choice in
                    Button {
        ...
        .task{
            if(pollVM.choices.count < 2){
                pollVM.getChoices(groupId: groupId, proposal: proposal)
            }
        }
        
        .alert(item:$pollVM.error) { error in
       
            Alert(title: Text("Failed to cast vote."), message: Text(error.description), dismissButton: .cancel() {
        ...
        // Switch between casting and removing vote
            ...
            .popover(isPresented: $pollVM.popOverCredInput){
                VStack(alignment: .center, spacing: 20){
                    ...
                    Button("Send"){
                        ...
                        pollVM.popOverCredInput = false
                        contentVM.sendingWriteTx = true
                        
                        if proposal.vote!.hasVoted {
                            // If we have voted then removeVote
                            pollVM.removeVote(groupId: groupId, proposalId: proposal.id!, password: password) { success in
                                ...
                                pollVM.choices[selection!].votes -= 1
                            ...
                            if selection != nil {
                                // Casting vote
                                pollVM.castVote(groupId: groupId, proposalId: proposal.id!, choiceId: selection!, password: "") {success in
                                ...
                                pollVM.error = CryptaError(description: "Select a poll option.")
                            }
                        }
...
struct PollView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        PollView(groupId: 0,proposal: Proposal(id: 0, title: "Test", description: "Test", creator: address, proposalStart: 0, proposalEnd: 0, choices: []), selection: 0)
    }
}
